...
- **Browse & filter** — 112 locations on a Leaflet map with cascading filters: choosing a district narrows the precinct and building lists, and choosing a precinct selects its parent district automatically (every precinct maps to exactly one district, so locality is derived, never typed)
